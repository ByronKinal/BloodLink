...
export function HowToDonateSection() {
  return (
    <section id="como-donar" className="fade-up px-16 py-20 bg-blanco">
      ...
      <div className="grid grid-cols-4 gap-[18px]">
        {STEPS.map(({ number, title, text, icon, badgeStyle, cardVariant }) => (
          <div
            key={number}
            className={`step-card ${cardVariant} bg-blanco rounded-2xl p-7 border border-gris2 transition-all duration-200 cursor-default hover:border-transparent hover:-translate-y-1 hover:shadow-[0_12px_32px_rgba(0,0,0,0.1)]`}
          >
            <div
              className="w-11 h-11 rounded-xl flex items-center justify-center font-cormorant text-[22px] font-medium mb-[18px]"
              style={badgeStyle}
            >
              {number}
            </div>
